...
class GLBitmap {
	...
	public static function renderMask (bitmap:Bitmap, renderSession:RenderSession):Void {
		...
		if (bitmap.bitmapData != null && bitmap.bitmapData.__isValid) {
			...
			//var shader = renderSession.shaderManager.initShader (bitmap.shader);
			var shader = renderSession.shaderManager.initShader (GLMaskManager.maskShader);
			renderSession.shaderManager.setShader (shader);
			...
			gl.bindBuffer (gl.ARRAY_BUFFER, bitmap.bitmapData.getBuffer (gl, 1, bitmap.__worldColorTransform));
